// global recipes dictionary that gets updated when display_recipes() is executed 
var recipes = {};

// Searches API for all matching recipes (from index.php)
function display_recipes() {
  var id = event.target.id; // id of category chosen (eg: if american is clicked, id=american)
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      // Hides original categories and dropdown menu button, and changes Discover Recipes to Category Recipes
      document.getElementById("categories").style.display = "none";
      document.getElementById("dropdownMenuButton").style.display = "none";
      document.getElementById("discover_recipes").innerText = id.charAt(0).toUpperCase() + id.slice(1) + " recipes";
    });
}

// submits form of specific recipe details to specific_recipe.php onclick
